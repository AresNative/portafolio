import { useEffect, useState } from "react"
import { ClimateData, consultClimate } from "../../services/climate"
import style from "./theme/card-climate.module.css"

function CardClimate() {
    const [cliamteData, setcliamteData] = useState<ClimateData | any>()
    useEffect(() => {
        consultClimate()
            .then((r: ClimateData) => {
                setcliamteData(r)
            })
    }, [])


    return <div className={style.cardContainer}>
        <div className={style.card}>
            <h1 className={style.city}>Baja California</h1>
            <h2 className={style.weather}>Ensenada</h2>
            <svg viewBox="0 0 100 100" height="50px" width="50px" y="0px" x="0px" id="Layer_1" version="1.1" className={style.weather}>
                <image href="data:image/png;base64," y="0" x="0" height="100" width="100" id="image0"></image>
            </svg>
            <p className={style.temp}>{Math.floor(cliamteData?.main.temp - 271)}°</p>
            <div className={style.minmaxContainer}>
                <div className={style.min}>
                    <p className={style.minHeading}>Min</p>
                    <p className={style.minTemp}>{Math.floor(cliamteData?.main.temp_min - 275)}°</p>
                </div>
                <div className={style.max}><p className={style.maxHeading}>Max</p>
                    <p className={style.maxTemp}>{Math.floor(cliamteData?.main.temp_max - 270)}°</p></div>
            </div>

        </div>
    </div>
}
export default CardClimate